...
import { openai, type CallAnalysisResult, type CallSummaryResult } from './openai'
...
export async function retrieveCallTranscript(
  ...
  try {
    ...
    if (existingTranscript && existingTranscript.processingStatus === 'completed') {
...
export async function generateAiAnalysisForTranscript(
  ...
  try {
    ...
    return await updateTranscriptWithAiAnalysis(transcript.id, transcript.content, true)
  } catch (error) {
 ...
 */
export async function processTranscript(transcript: Transcript): Promise<Transcript> {
 ...
 */
async function processTranscriptWithAi(transcript: Transcript): Promise<Transcript> {
  try {
    ...
    try {
      ...
      const aiResults = await openai.generateComprehensiveAnalysis(transcript.content)
...
export function validateTranscriptContent(content: string): {
  ...
  if (hasIncomplete) {
    warnings.push('Transcript contains markers indicating incomplete or unclear audio')
  }
...
function calculateTranscriptConfidence(content: string): number {
  ...
  const incompleteCount = incompleteMarkers.reduce((count, marker) => {
    return count + (content.toLowerCase().match(new RegExp(marker.toLowerCase(), 'g'))?.length || 0)
  }, 0)
...
function detectLanguage(content: string): string {
  ...
  const englishCount = englishWords.reduce((count, word) => {
    return count + (content.toLowerCase().match(new RegExp(`\\b${word}\\b`, 'g'))?.length || 0)
  }, 0)
...
function countSpeakers(content: string): number {
  ...
  const speakerMatches = content.match(/(Speaker|User|Assistant|Agent)\s*\d*/gi) || []
  const uniqueSpeakers = new Set(speakerMatches.map(match => match.toLowerCase().trim()))
