...
describe('Error Handling System', () => {
  ...
  describe('ApiErrorHandler', () => {
    ...
    it('should not retry non-retryable errors', async () => {
      ...
      await expect(
        errorHandler.executeWithRetry(mockOperation, 'test-service', 'test-operation')
    ...
    it('should provide health status', async () => {
      ...
      await freshErrorHandler.executeWithRetry(successOp, 'healthy-service', 'test')
      await freshErrorHandler.executeWithRetry(successOp, 'healthy-service', 'test')
      
      try {
        await freshErrorHandler.executeWithRetry(failOp, 'degraded-service', 'test')
      } catch (error) {
        // Expected to fail
      }
      await freshErrorHandler.executeWithRetry(successOp, 'degraded-service', 'test')
    ...
    it('should track metrics correctly', async () => {
      ...
      await errorHandler.executeWithRetry(successOp, 'metrics-test', 'success-op')
  ...
  describe('Error Classification', () => {
    ...
    it('should retry rate limit errors', async () => {
      ...
      const result = await errorHandler.executeWithRetry(mockOperation, 'test', 'test')
